                        ...
                        <div class="card mb-3">
                            ...
                            <div class="card-body">
                              ...
                              <div class="card-btns">
                                ...
                                <i class="far fa-heart fav" data-id="${product.id}" onClick="addToFav(${product.id})"></i>
                              </div>
                            </div>
                          </div>
        </div>
...
let favItems=getFavourites?[...getFavourites]:[]
if(getFavourites){
  getFavourites.map((item)=>{
    let heart=document.querySelector(`.fav[data-id="${item.id}"]`)
...
function addToFav(id){
  if(localStorage.getItem("email")){
    ...
    let heart=document.querySelector(`.fav[data-id="${favItem.id}"]`)
    let heartStyle=window.getComputedStyle(heart)
    if(heartStyle.getPropertyValue("color") === "rgb(255, 0, 0)"){
      heart.style.color="rgb(128, 128, 128)"
...
searchInput.addEventListener("input", function(){
  if(searchInput.value !==""){
    if(searchSelect.value === "name"){
      let selectedItem=products.filter((item)=>item.name.toLowerCase().includes(searchInput.value.toLowerCase()))
      draw(selectedItem)
    }
    else if(searchSelect.value === "category"){
      let selectedItem=products.filter((item)=>item.category.includes(searchInput.value.toLowerCase()))
      draw(selectedItem)
    }
  }
  else{
